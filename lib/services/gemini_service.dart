...
const _kFallbackApiKey = geminiApiKey;
...
class GeminiService {
  ...
  // --- Firebase 비활성 환경 폴백 (직접 HTTP 호출) ---

  static Future<Map<String, dynamic>> _callGemini(String prompt) async {
    final uri = Uri.parse('$_kGeminiEndpoint?key=$_kFallbackApiKey');
    final body = jsonEncode({
      'system_instruction': {
        'parts': [{'text': _kSystemInstruction}]
      },
      'contents': [
        {
          'parts': [{'text': prompt}]
